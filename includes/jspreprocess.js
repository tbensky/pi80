...
function js_preprocess(code_in)
{
	...
	var while_condition_start, whilst_condition_start;
	var whilst_condition;
	var while_condition;
	var i=0;
	var code_out = '';
	var ws = ' \n\r\t';
			...
			if (!in_dq && !in_sq)
				{
					...
					if (look_right(code_in,i,"end") && in_body_of.last() == 'whilst')
						{
							in_body_of.pop();
							code_out += '\n if ('+whilst_cond+') { run_whilst(); }\n}\ncore_whilst();\n\n';
							i += ws_length("end");
						}
					...
					if (look_right(code,i,"whilst"))
						{
							in_whilst_condition = true;
							i += ws_length("whilst");
							whilst_condition_start = i;
						}
						
					if (look_right(code,i,"do") && in_whilst_condition)
						{
							in_whilst_condition = false;
							in_body_of.push('whilst');
							whilst_cond = code_in.substr(whilst_condition_start,i - whilst_condition_start);
							i += ws_length("do");
							code_out += 'function core_whilst()\n{\n';
						}
		 	...
		 	if (i < code_in.length)
		 		{
		 			if (!in_if_condition && !in_while_condition && !in_whilst_condition && in_for_limits == 0)
						code_out += code_in[i];
					i++;
				}
		}
